...
import {
    ArrowUpRight, BarChart2, Calendar, LineChart as LineChartIcon,
    Minus, Plus, TrendingDown, TrendingUp
} from 'lucide-react';
import MetricsSummary from "./MetricsSummary";
import {useAnalyticsData} from "./fetchAnalyticsData";

// Helper components
const DateDisplay = ({date, value, icon: Icon, color}) => (
...
const MetricItem = ({label, value, color, trend}) => (
...
const ChartRenderer = ({type, data, theme, onHover}) => {
    ...
    const commonProps = {
        ...
        onTouchMove: (e) => e.activePayload && onHover(e.activePayload[0].payload),
...
const BookingTrendsChart = ({selectedDateRange}) => {
        ...
        >
            <div className="p-4 space-y-4">
                ...
                <div className="flex justify-center gap-1 p-1 bg-gray-50 rounded-lg">
                    {Object.values(CHART_TYPES).map(({id, label, icon: Icon}) => (
                        <button
                            key={id}
                ...
                <div className="relative">
                    ...
                    {/* Zoom Controls */}
                    <div className="absolute bottom-2 right-2 flex items-center gap-1 p-1 bg-white rounded-lg shadow-md">
                        <button
